...
export function CallCenterProvider({ children }: { children: React.ReactNode }) {
  ...
  const finishSession = useCallback(async (session: CallSession, status: 'ended' | 'failed' = 'ended') => {
    ...
    const log: CallLog = {
      ...
      customerNumber: session.log?.customerNumber || session.phone,
      fromNumber: session.log?.fromNumber || (session.direction === 'outbound' ? user?.id || '' : session.phone),
      toNumber: session.log?.toNumber || (session.direction === 'outbound' ? session.phone : user?.id || ''),
  ...
  const attachCallEvents = useCallback((call: StringeeCallLike, session: CallSession) => {
    ...
    const markAnswered = async () => {
      ...
      const saved = await callCenterService.saveLog({
        ...
        status: 'answered',
        customerNumber: trackedSession.log?.customerNumber || trackedSession.phone,
        fromNumber: trackedSession.log?.fromNumber || (trackedSession.direction === 'outbound' ? user?.id || '' : trackedSession.phone),
        toNumber: trackedSession.log?.toNumber || (trackedSession.direction === 'outbound' ? trackedSession.phone : user?.id || ''),
  ...
  const answerActiveCall = useCallback(async () => {
    ...
    try {
      ...
      const saved = await callCenterService.saveLog({
        ...
        fromNumber: activeCall.log?.fromNumber || user?.id || '',
        toNumber: activeCall.log?.toNumber || activeCall.phone,
