...
const DEFAULT_METTA_PLUS_CONFIG: MettaPlusConfig = {
  ...
  passItems: [
    'Tham gia CLB phù hợp độ tuổi',
    'Học kỹ năng với GVNN',
    'Thực hành STEM Robotics',
    'Có Metta Passport cá nhân',
    'Tham gia Showcase cuối khóa',
  ],
...
function parseMettaPlusConfig(section?: PageSection): MettaPlusConfig {
  if (!section?.extraData) return DEFAULT_METTA_PLUS_CONFIG;
  try {
    const parsed = JSON.parse(section.extraData) as Partial<MettaPlusConfig>;
    return {
      ...DEFAULT_METTA_PLUS_CONFIG,
      ...parsed,
      heroTags: parsed.heroTags?.length ? parsed.heroTags : DEFAULT_METTA_PLUS_CONFIG.heroTags,
      benefits: parsed.benefits?.length ? parsed.benefits : DEFAULT_METTA_PLUS_CONFIG.benefits,
      ageGroups: parsed.ageGroups?.length ? parsed.ageGroups : DEFAULT_METTA_PLUS_CONFIG.ageGroups,
      passItems: parsed.passItems?.length ? parsed.passItems : DEFAULT_METTA_PLUS_CONFIG.passItems,
      journey: parsed.journey?.length ? parsed.journey : DEFAULT_METTA_PLUS_CONFIG.journey,
      reasons: parsed.reasons?.length ? parsed.reasons : DEFAULT_METTA_PLUS_CONFIG.reasons,
      formHighlights: parsed.formHighlights?.length ? parsed.formHighlights : DEFAULT_METTA_PLUS_CONFIG.formHighlights,
      heroImage: parsed.heroImage || section.imageUrl || DEFAULT_METTA_PLUS_CONFIG.heroImage,
    };
  } catch {
    return DEFAULT_METTA_PLUS_CONFIG;
  }
}
...
function MiniCard({ title, desc, icon, color }: { title: string; desc: string; icon: IconName; color: ColorKey }) {
  const Icon = getIcon(icon);
  return (
    <article className={`rounded-[28px] p-5 shadow-[0_18px_42px_-24px_rgba(8,36,74,.35)] ring-1 ${colorClasses[color]}`}>
...
function MettaPlusForm({ ctaText }: { ctaText: string }) {
...
export default function MettaPlusLanding() {
  const [config, setConfig] = useState(DEFAULT_METTA_PLUS_CONFIG);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const section = sections.find((item) => item.type === 'Metta+ Landing')
        || seedSections.find((item) => item.pageId === 'page-metta-plus' && item.type === 'Metta+ Landing');
      if (active) setConfig(parseMettaPlusConfig(section));
    }
    load().catch(() => {
      if (active) setConfig(DEFAULT_METTA_PLUS_CONFIG);
    });
    return () => { active = false; };
  }, []);
          ...
          <div className="mx-auto max-w-[1180px]">
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              {config.ageGroups.map((card) => {
                ...
                return (
                <article key={card.title} className={`min-h-[210px] rounded-[32px] p-6 shadow-[0_18px_42px_-24px_rgba(8,36,74,.35)] ${solidClasses[card.color]}`}>
            ...
            <div className="grid gap-4 lg:grid-cols-4">
              {config.journey.map((step, index) => {
                ...
                <article key={step.title} className="relative rounded-[28px] bg-white p-5 shadow-[0_18px_42px_-24px_rgba(8,36,74,.35)] ring-1 ring-[#E8EEF7]">
                  <div className={`mb-5 flex h-[52px] w-[52px] items-center justify-center rounded-2xl ${colorClasses[step.color]}`}>
        ...
        <section id="metta-plus-form" className="relative bg-[#EAF5FF] px-5 py-14 sm:px-6 lg:py-18">
          ...
          <div className="relative mx-auto grid max-w-[1060px] items-center gap-8 rounded-[40px] bg-white p-5 shadow-[0_34px_70px_-36px_rgba(8,36,74,.65)] ring-1 ring-[#DCE9F8] sm:p-8 lg:grid-cols-[0.88fr_1.12fr] lg:p-10">
            ...
            <div className="rounded-[30px] bg-[#F8FBFF] p-5 ring-1 ring-[#DCE9F8] sm:p-6">
              <MettaPlusForm ctaText={config.formCta} />
            </div>
          </div>
        </section>
      </main>

      <footer className="bg-[#08244A] px-5 py-8 text-white sm:px-6">
        <div className="mx-auto flex max-w-[1180px] flex-col items-center justify-between gap-4 text-center sm:flex-row sm:text-left">
          <div className="flex items-center gap-3">
            <span className="rounded-2xl bg-white p-2"><img src={LOGO} alt="METTA Academy" className="h-9 w-auto" /></span>
            <div>
              <p className="font-montserrat text-lg font-extrabold">METTA Academy</p>
              <p className="text-[13px] font-semibold text-white/65">{config.footerSubtitle}</p>
            </div>
          </div>
          <button type="button" onClick={scrollToForm} className="rounded-full bg-[#F37021] px-6 py-3 text-[14px] font-extrabold shadow-lg transition hover:-translate-y-0.5">
            {config.footerCta}
          </button>
        </div>
      </footer>
